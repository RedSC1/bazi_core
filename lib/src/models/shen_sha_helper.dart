...
class ShenShaInfo {
  ...
  /// 命宫神煞列表
  final List<String> mingGongShenSha;

  /// 身宫神煞列表
  final List<String> shenGongShenSha;

  /// 胎元神煞列表
  final List<String> taiYuanShenSha;

  /// 胎息神煞列表
  final List<String> taiXiShenSha;
  ...
  const ShenShaInfo({
    ...
    this.mingGongShenSha = const [],
    this.shenGongShenSha = const [],
    this.taiYuanShenSha = const [],
    this.taiXiShenSha = const [],
  ...
  String toString() {
    ...
    if (mingGongShenSha.isNotEmpty) {
      buf.writeln('命宫: ${mingGongShenSha.join(", ")}');
    }
    if (shenGongShenSha.isNotEmpty) {
      buf.writeln('身宫: ${shenGongShenSha.join(", ")}');
    }
    if (taiYuanShenSha.isNotEmpty) {
      buf.writeln('胎元: ${taiYuanShenSha.join(", ")}');
    }
    if (taiXiShenSha.isNotEmpty) {
      buf.writeln('胎息: ${taiXiShenSha.join(", ")}');
    }
...
class ShenShaHelper {
  ...
  static ShenShaInfo analyze(
    ...
    return ShenShaInfo(
      ...
      mingGongShenSha: extraPillars.enableMingGong
          ? getShenSha(chart, chart.mingGong, PillarType.mingGong)
          : const [],
      shenGongShenSha: extraPillars.enableShenGong
          ? getShenSha(chart, chart.shenGong, PillarType.shenGong)
          : const [],
      taiYuanShenSha: extraPillars.enableTaiYuan
          ? getShenSha(chart, chart.taiYuan, PillarType.taiYuan)
          : const [],
      taiXiShenSha: extraPillars.enableTaiXi
          ? getShenSha(chart, chart.taiXi, PillarType.taiXi)
          : const [],
