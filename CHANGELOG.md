...
- 完成四柱、关系、神煞、起运运表和反查的 JS／C++／Dart Web 验证。

## 1.0.0-beta.1

- **[重大破坏性更新]** 基于 `bazi-lite` 重写为纯 Dart 实现，公共 API 与旧版 `0.6.x` 不兼容；`TimePack` 拆分为显式的物理时刻、计算钟表和逐盘选项，默认采用民用钟表时间。迁移方式见 `doc/migration.md`。
- **[依赖重构]** 天文与历法内核由 `sxwnl_spa_dart` 迁移至已发布的 `ephemeris_lite`。
- **[功能移植]** 提供四柱规则、66 种神煞、关系汇总、起运／运表、人元司令及分步／完整反查。
- **[API 完善]** 补齐公开常量、快捷别名、`JulianTime` 输入、JSON 选项恢复及 JS → Dart 接口对照表。
- **[状态模型]** 增加逐盘不可变选项、物理时刻与计算钟表分离，以及不可变神煞模块目录和选择快照；内置规则可停用但不可覆盖或删除。
- **[验证]** 覆盖共享 C++ 夹具、JS 数值对照、旧 Dart 规则回归和 Dart Web 运行验证。
- **[许可证]** 新实现沿用 `bazi-lite` 的 MPL-2.0；旧 MIT 实现保留在此前版本历史中。
...
- **[依赖升级]** 升级 `sxwnl_spa_dart` 至 `^0.18.4`。
