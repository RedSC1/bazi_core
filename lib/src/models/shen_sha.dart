...
/// 【天医】(常用现代表：月建前一位)
/// 规则：以月支查地支，取月支前一位(逆行一位)。例如寅月见丑。
/// 传本中也存在其他天医表；本实现固定采用这一现代四柱查法。
...
final List<ShenSha> shenShaRegistry = [
  ...
  // -------------------------
  // 1.5 羊刃 (Stem -> Branch)
  // 规则：本实现只以日干查目标地支，并采用“帝旺位”表。
  // 阳干：甲卯、丙戊午、庚酉、壬子。
  // 阴干（本项目采用的逆行长生帝旺变体）：乙寅、丁己巳、辛申、癸亥。
  // 《三命通会》论阳刃另有“惟五阳干有刃”的传承，阴干表不可视为唯一正统。
  // -------------------------
  StemToBranchShenSha(
    '羊刃',
    {
  ...
  // 口诀：正月见丑，二月见寅...（月建前一位，逆行一位）。
  // 当前采用常见现代四柱表；历史传本存在不同天医表。
  // -------------------------
  TianYiShenSha('天医'),
  ...
  // -------------------------
  // 1.27 天罗地网
  // 规则：当前采用纳音年命、性别和成对地支的传统变体。
  // 戌亥为天罗，辰巳为地网。
  // 纳音火命人，男命见戌亥为天罗；
  // 纳音水土命人，女命见辰巳为地网；
  // 金木命人无。
  // 传统传本也有不加性别、或以日干而非年命纳音判定的版本。
  // -------------------------
  ...
  // “天转/地转”的命名、查日柱还是时柱，在不同传表中有交换；本实现
  // 允许任意目标柱，按上述表匹配。
  // -------------------------
  DiZhuanShenSha('地转'),
